...
class TransformControls extends Object3D {
  constructor(camera, id, core, renderNode) {
    super();
    ...
    this._core.setEvenet("gl" + "pointerdown", this._id, this._onPointerDown);
    this._core.setEvenet("gl" + "pointermove", this._id, this._onPointerHover);
    this._core.setEvenet("gl" + "pointerup", this._id, this._onPointerUp);
  }
...
function getPointer(event) {
  const domElement = this._renderNode.node.domElement.parentElement;
...
function onPointerDown(event) {
  if (!this.enabled) return;
  let element= this._renderNode.node.domElement
  if (!element.pointerLockElement) {
    element.setPointerCapture(event.pointerId);
  }

  this._core.setEvenet("gl" + "pointermove", this._id, this._onPointerMove);
...
function onPointerUp(event) {
  ...
  this._renderNode.node.domElement.releasePointerCapture( event.pointerId );
