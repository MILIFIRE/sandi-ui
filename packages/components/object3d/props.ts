...
const mapFn = (watchSet: Set<WatchStopHandle>, props: any, instance: Object3D, mapKey: string, key1: string, key2: string) => {
    if (instance[key1]) {
        if (props[mapKey]) {
            instance[key1][key2] = props[mapKey]
        }
        watchWrap(watchSet, watch(() => props[mapKey], (val: any, oldVal: any) => {
            instance[key1][key2] = val;
        }))
    }
}
export const propsWatch = (props: any, instance: Object3D): Set<WatchStopHandle> => {
    ...
    const mapProps = {
        ...
        'scaleXYZ': (props: any, instance: any, val: any, oldVal: any) => {
            ...
            watchWrap(watchSet, watch(() => props['scaleXYZ'], (val: any, oldVal: any) => {
                instance.scale.set(val, val, val);
            }))
        }
    }
    ...
    for (let propKey in object3dProps) {
        if (instance[propKey]) {
            ...
            watchWrap(watchSet, watch(() => props[propKey], (val: any) => {
                if (isFunction(instance[propKey])) {
